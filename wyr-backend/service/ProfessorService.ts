...
professorService.get("/all", async (_, res) => {
  ...
  if (!listOfProfessors || listOfProfessors.length === 0) {
    res
      .status(404)
      .send({ message: "No professors were found in the database" });
    return;
  }
...
professorService.get("/:id", async (req, res) => {
  ...
  if (!professor) {
    res
      .status(404)
      .send({
        message: `No professor was found in the database with the specified id ${id}`,
      });
    return;
  }
...
professorService.delete("/:id", async (req, res) => {
  ...
  if (!professor) {
    res
      .status(404)
      .send({
        message: `No professor were found in the database with the specified id ${id}`,
      });
    return;
  }
  res.status(200).send(professor);
